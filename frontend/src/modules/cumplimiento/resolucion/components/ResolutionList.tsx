...
export default function ResolutionList({ resolutions: initialResolutions, onEdit }: ResolutionListProps) {
    ...
    return (
        <div className="bg-white dark:bg-[#151824] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.25)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.5)] p-6">
            ...
            {/* Tabla - Mantén el resto igual que antes */}
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {displayResolutions.map((resolution) => (
                            <tr key={resolution.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                ...
                                <td className="py-4 px-4">
                                    {resolution.attachment ? (
                                        <a href={resolution.attachment} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 hover:text-blue-600 hover:underline flex items-center gap-1">
                                ...
                                <td className="py-4 px-4">
                                    {resolution.url ? (
                                        <a href={resolution.url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 hover:text-blue-600 hover:underline">
                                ...
                                <td className="py-4 px-4 sticky right-0 bg-white dark:bg-[#151824]">
                                    <div className="flex items-center justify-center gap-1">
                                        {resolution.url && (
                                            <a href={resolution.url} target="_blank" rel="noopener noreferrer" className="p-2 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-lg transition-colors" title="Ver documento">
                                                <Icon icon="mdi:open-in-new" width="18" height="18" className="text-blue-500" />
                                            </a>
                                        )}
                                        <button onClick={() => onEdit(resolution)} className="p-2 hover:bg-amber-100 dark:hover:bg-amber-900/30 rounded-lg transition-colors" title="Editar">
                                            <Icon icon="mdi:pencil" width="18" height="18" className="text-amber-500" />
                                        </button>
                                        <button onClick={() => handleDelete(resolution.id)} className="p-2 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors disabled:opacity-50" title="Eliminar" disabled={deleteResolution.isPending}>
